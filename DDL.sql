...
-- 데이터 정의어 (DDL)
-- 데이터베이스, 테이블, 사용자 등(스키마)을 정의하는 데 사용되는 언어
...
-- 데이터베이스 사용: 데이터베이스 작업을 수행하기 전에 반드시 작업할 데이터베이스를 선택데이터를 선택해야 한다
USE practice_sql;
...
CREATE USER 'developer'@'192.168.7.186' IDENTIFIED BY 'P!ssw0rd';
...
-- 테이블 삭제
-- 만약 해당 테이블을 참조하고 있는 다른 테이블이 존재하면 테이블 삭제가 불가능example_tableexample_table
