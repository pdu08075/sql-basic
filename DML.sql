USE practice_sql; -- USE 적거나 좌측 데이터베이스 직접 마우스 클릭하면 이동
-- 데이터 조작어 (DML)
-- 테이블에 레코드를 삽입, 조회, 수정, 삭제할 때 사용alter
...
-- SELECT: 테이블에서 레크도르르 조회할 떄 사용하는 명령어
...
-- UPDATE: 테이블에서 레코드를 수정할 떄 사용하는 명령어
...
-- INSERT INTO SELECT: 삽입 작업 시에 조회 결과를 사용
