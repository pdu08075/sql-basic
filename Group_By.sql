...
-- 집계 함수: 여려 행의 레코드를 종합하여 하나의 결과값을
...
-- 집계 함수가 포함되어 있는 쿼리 혹은 그룸화가 되어 있는 쿼리에서는
-- 그룹화한 컬럼과 제외한 컬럼은 SELECT 절에서 사용이 불가능
...
-- 주의!! WHERE 절과 사용 방법이 비슷하지만 WHERE 절은 조회할 떄 사용, HAVING 절은 조회 결과에 사용
SELECT
	AVG(amount) '평균',
    employee_number '사번'
FROM sale
