...
-- LEFT OUTER JOIN (LEFT JOIN): 기준 테이블의 모든 레코드와 조합할 테이블 중 일치하는 레코드만 반환
...
-- RIGHT OUTER JOIN (RIGHT JOIN) : 조합할 테이블의 모든 레코드와 기준 테이블 중 조건에 일치라는 레코드만 반환
...
-- CROSS JOIN 결과 레코드 수 = 기준 테이블 레코드
SELECT *
...
-- 쿼리 내부에 존재하는 또 다른 쿼리, 쿼리 결과를 조건이나테이블로 사용할 수 있도록 함
